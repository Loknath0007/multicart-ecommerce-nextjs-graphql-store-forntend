...
const GET_PAGES = gql`
  query getPages($indexFrom: Int) {
    getPages(indexFrom: $indexFrom) {
      title
      description
      slug
      createdAt
...
const SideBar = () => {
  var { loading, data, refetch } = useQuery(GET_PAGES, {
    variables: { indexFrom: 0 },
  });
  console.log("data:"), data?.getPages;
  const closeNav = () => {
